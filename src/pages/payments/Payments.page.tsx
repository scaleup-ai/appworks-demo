...
const PaymentsPage: React.FC = () => {
  ...
  const handleTestPaymentReconciliation = async () => {
    ...
    try {
      ...
      setReconciliationTests(prev => 
        prev.map(test => 
          test.id === newTest.id 
            ? { ...test, result, status: 'completed' as const }
            : test
        )
      );
  ...
  const handleRunBulkReconciliation = async () => {
    try {
      // Get some invoices to create test payments for
      const invoices = await accountsReceivablesApi.listInvoices({ limit: 5 });
            ...
            <div className="overflow-x-auto">
              ...
              {reconciliationTests.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  No reconciliation tests performed yet
                </div>
              )}
            </div>
          )}
