...
const CashFlowPage: React.FC = () => {
  ...
  const { execute: loadCashFlowData, isLoading: loading } = useApi(fetchCashFlowData, {
    errorMessage: "Failed to load cash flow data",
  });
  ...
  const handleExportForecast = () => {
    const exportData = { summary, forecast, scenarios, generatedAt: new Date().toISOString(), selectedScenario };
    downloadJson("cash-flow-forecast.json", exportData);
    showToast("Cash flow forecast exported", { type: "success" });
  };

  const handleScenarioChange = (scenarioId: string) => {
    setSelectedScenario(scenarioId);
    const scenario = scenarios.find((s) => s.id === scenarioId);
    if (scenario) showToast(`Viewing ${scenario.name} scenario`, { type: "info" });
  };
