...
const CollectionsPage: React.FC = () => {
  ...
  const handleSelectAll = () => {
    if (selectedInvoices.size === invoices.filter(inv => inv.status !== "PAID" && (inv.daysPastDue || 0) > 0).length) {
      setSelectedInvoices(new Set());
    } else {
      ...
      <DashboardLayout title="Collections">
        <div className="py-12">
          <div className="max-w-md mx-auto p-6 border rounded-lg bg-yellow-50 text-center">
            ...
            <p className="mt-2 text-sm text-yellow-700">Connect your Xero account to access collections and invoice data.</p>
            <div className="mt-4">
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <SummaryCardGrid
          items={[
            { title: "Total Outstanding", value: formatCurrency(summary.totalOutstanding), className: "border-l-4 border-l-blue-500" },
            { title: "Overdue Amount", value: formatCurrency(summary.overdueAmount), className: "border-l-4 border-l-red-500" },
            { title: "Current Amount", value: formatCurrency(summary.currentAmount), className: "border-l-4 border-l-green-500" },
            { title: "Scheduled Reminders", value: summary.scheduledReminders, className: "border-l-4 border-l-purple-500" },
          ]}
        />
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {selectedInvoices.size > 0 && (
                <span className="text-sm text-gray-600">{selectedInvoices.size} invoice{selectedInvoices.size !== 1 ? "s" : ""} selected</span>
              )}
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Select</th>
                  ...
                  <th className="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Days Past Due</th>
                  <th className="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Stage</th>
                ...
                {invoices
                  ...
                  .map((invoice) => (
                    <tr key={invoice.invoiceId} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : "No due date"}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{(invoice.daysPastDue || 0) > 0 ? invoice.daysPastDue : "-"}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <StatusBadge
                          variant={
                            invoice.reminderStage === "current" ? "green" :
                            invoice.reminderStage === "pre_due" ? "blue" :
                            invoice.reminderStage === "overdue_stage_1" ? "yellow" :
                            invoice.reminderStage === "overdue_stage_2" ? "orange" :
                            invoice.reminderStage === "overdue_stage_3" ? "red" : "gray"
                          }
                        >
